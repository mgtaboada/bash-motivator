...
# Installation
    ...
    git clone https://github.com/mgtabodada/bash-animator.git bash-animator
    sudo cp bash-animator/src/bash.command-not-found /etc/

    # Method 2 - I don't care, insult me!
    sudo wget -O /etc/bash.command-not-found https://raw.githubusercontent.com/hkbakke/bash-animator/master/src/bash.command-not-found
...
```
if [ -f /etc/bash.command-not-found ]; thenpppp
...
# Configuration
bash-animator can be customized, or even be made polite and nice, by populating `CMD_NOT_FOUND_MSGS` or `CMD_NOT_FOUND_MSGS_APPEND` environment variables. The values should be arrays. `CMD_NOT_FOUND_MSGS` replaces the default messages, while `CMD_NOT_FOUND_MSGS_APPEND` appends more messages to the existing ones.
